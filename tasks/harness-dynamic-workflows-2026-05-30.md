...
1. **Budget unit — COST or RAW tokens?** There are three "totals" in-tree (runner cap = in+out only; durable record = in+out+cache; `estimate_cost` = weighted incl. cache/reasoning). Recommendation: **COST (micro-USD)** so `FLEET=floor(total/100000)` and `remaining()>50000` are spend thresholds, with `max_total_tokens` staying a separate per-runner in+out cap. Confirm this is how budgets should be expressed on the CLI.
...
4. **Typed schema ergonomics — gate `schema::<T>()` behind an optional `derive-schema`/schemars feature (with `schema_value(Value)` always available), or make schemars a hard dep?** schemars stack-overflows on recursive types with `inline_subschemas`, and 1.x vs 0.8 APIs differ.
