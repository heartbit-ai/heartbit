...
5. **Resume determinism contract — documented discipline vs enforced?** ✅ **RESOLVED in P4 → documented discipline + a content-addressed key that is sound by construction** (`flow/journal.rs`). Decisions that differ from the original sketch, with rationale:
   - **Content-addressed, not strict positional prefix.** Key = `(content_hash, occurrence)` where `content_hash` = SHA-256 over the call's *inputs* (prompt, requested model, schema) and `occurrence` disambiguates byte-identical calls. A global call-index would shift the whole tail when one early call is inserted/removed; content addressing keeps each unchanged call's key stable, and **data dependencies propagate invalidation for free** (a changed call's successors get different prompts → different hashes → re-run).
   - **Hash inputs only, never `AgentOutput.model_name`** — that's an *output* (the cascade may escalate), so hashing it would defeat replay after an escalation.
   - **`run_id` via `derive_run_id` (SHA-256), not `uuid::new_v5`** — keeps "same inputs ⇒ same journal file" with zero new deps (`uuid` is v4-only here; `v5` would be a new feature).
   - **Canonicalize before hashing regardless of `preserve_order`.** The report assumed `serde_json` `preserve_order` is ON; it is actually **OFF** in this workspace — but `canonical_json` (recursive `BTreeMap` key-sort) runs anyway, so reordered-but-equal schemas always hash identically even if a transitive Cargo feature later flips `preserve_order` on. (Test: `reordered_schema_keys_still_replay`.)
   - **Soundness scope is documented, not enforced** (Rust can't compile-forbid `Instant::now()`/`rand` in a workflow body): journal restores *return values, not side effects* (so P5 will gate `isolation != None` agents out of the journal); single-process / single-journal / fixed-provider; and under `parallel()`/`pipeline()` the occurrence counter races, degrading to a safe **false-miss** (a raced call re-runs) — never a false hit. The journal HIT is **leaf step 0/1, before permit+backstop+budget**, so a replay costs **0 work and 0 budget** (tests: `resume_replays_without_calling_provider`, `replayed_prefix_costs_zero_budget`). A torn final JSONL line from a crash is skipped on resume.
...
10. **`/workflows` restart-one — handle-exposing `parallel_with_controls` vs stop-all + journal-resume?** ✅ **RESOLVED in P6 → stop-all + journal-resume; per-task `AbortHandle` deferred.** Shipped controls: `WorkflowCtx::stop()` fires the shared `CancellationToken` (cooperative — agents wind down at the leaf's step-0 cancel-check). "Restart" is re-running with a resume `journal()`: every completed agent replays from the journal (0 work/0 spend), and only the stopped/uncompleted agents run live — which is strictly more useful than a raw task-abort because it preserves *all* prior work, not just one branch. A `parallel_with_controls(..) -> (fut, Vec<AbortHandle>)` for aborting a *single* in-flight agent without stopping the run is deferrable: the `JoinSet` in `parallel`/`pipeline` already gives abort-on-drop, and surfacing per-leaf handles adds API surface for a marginal case. Defer until a concrete need. Also resolved here: **P6 observability** = `flow::progress::{ProgressTracker, RunProgress, PhaseProgress}` folds `WorkflowEvent`s (exhaustive match) into a `/workflows`-style snapshot (phase list, agent counts, replayed/skipped/failed tallies, token totals); **one-level `workflow()` nesting** shares the parent's budget/journal/cap/backstop/cancel via `WorkflowCtx::nested()` (a 2nd level → `Error::Config`), with an independent default-phase per level.
