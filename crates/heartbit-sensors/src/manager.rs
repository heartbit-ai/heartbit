...
use crate::sources::audio::AudioSensor;
use crate::sources::image::ImageSensor;
use crate::sources::jmap::JmapEmailSensor;
use crate::sources::mcp::{McpSensor, McpTriageProcessor};
use crate::sources::rss::RssSensor;
use crate::sources::weather::WeatherSensor;
use crate::sources::webhook::WebhookSensor;
use crate::stories::StoryCorrelator;
use crate::triage::audio::AudioTriageProcessor;
use crate::triage::context::{TaskContext, TrustLevel};
use crate::triage::email::EmailTriageProcessor;
use crate::triage::image::ImageTriageProcessor;
use crate::triage::rss::RssTriageProcessor;
use crate::triage::structured::StructuredTriageProcessor;
use crate::triage::webhook::WebhookTriageProcessor;
use crate::triage::{TriageDecision, TriageProcessor};
use crate::{Sensor, SensorConfig, SensorEvent, SensorSourceConfig};
            ...
            msg_result = consumer.recv() => {
                ...
                match decision {
                    TriageDecision::Promote {
                        ...
                        let cmd = heartbit::daemon::types::DaemonCommand::SubmitTask {
                            id: uuid::Uuid::new_v4(),
...
mod tests {
    ...
    fn build_sensors_from_jmap_email_config() {
        ...
        assert_eq!(sensors[0].modality(), crate::SensorModality::Text);
    }
    ...
    fn build_triage_processors_from_jmap_email_config() {
        ...
        assert_eq!(processors[0].modality(), crate::SensorModality::Text);
    }
    ...
    fn build_sensors_from_image_config() {
        ...
        assert_eq!(sensors[0].modality(), crate::SensorModality::Image);
    }
    ...
    fn build_triage_processors_from_image_config() {
        ...
        assert_eq!(processors[0].modality(), crate::SensorModality::Image);
    }
    ...
    fn build_sensors_from_audio_config() {
        ...
        assert_eq!(sensors[0].modality(), crate::SensorModality::Audio);
    }
    ...
    fn build_triage_processors_from_audio_config() {
        ...
        assert_eq!(processors[0].modality(), crate::SensorModality::Audio);
    }
    ...
    fn build_sensors_from_weather_config() {
        ...
        assert_eq!(sensors[0].modality(), crate::SensorModality::Structured);
    }
    ...
    fn build_triage_processors_from_weather_config() {
        ...
        assert_eq!(processors[0].modality(), crate::SensorModality::Structured);
    }
    ...
    fn promoted_event_produces_valid_daemon_command() {
        ...
        match parsed {
            heartbit::daemon::types::DaemonCommand::SubmitTask { task, source, .. } => {
    ...
    fn promoted_event_includes_story_id() {
        ...
        let parsed: heartbit::daemon::types::DaemonCommand =
            serde_json::from_slice(&payload).unwrap();
        match parsed {
            heartbit::daemon::types::DaemonCommand::SubmitTask { story_id, .. } => {
    ...
    fn build_sensors_from_mcp_config() {
        let manager = make_manager(vec![SensorSourceConfig::Mcp {
            ...
            modality: crate::SensorModality::Text,
            poll_interval_seconds: 60,
    ...
    fn task_context_built_from_promote_decision_fields() {
        use crate::triage::{ActionCategory, Priority};
