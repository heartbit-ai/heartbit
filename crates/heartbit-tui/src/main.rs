        ...
        for effect in std::mem::take(&mut app.effects) {
            match effect {
                Effect::SendInput(text) => {
                    if !agent_started {
                        // Build the runner on the agent thread (MCP transports bind
                        // to their spawn runtime). `agent_started` is set optimistically
                        // and reset when the thread exits via `RunCompleted` (below),
                        // so a build failure (bad key, MCP down) lets the user retry.
                        agent_started = true;
                        ...
                        std::thread::spawn(move || {
                            // multi_thread (≥2 workers): a blocking tool body can never
                            // starve the scheduler and stall the interrupt cancel poll.
                            ...
                            rt.block_on(async move {
                                ...
                                {
                                    Ok(runner) => {
                                        let _ = runner.execute(&text).await;
                                    }
                                    Err(e) => {
                                        let _ = done_tx
                                            .send(Msg::Notice(format!("cannot start agent: {e}")));
                                    }
                                }
                                // Signals the UI to reset `agent_started` so the next
                                // message rebuilds (covers build failure and session end).
