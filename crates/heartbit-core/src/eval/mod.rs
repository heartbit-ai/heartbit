...
pub struct EvalRunner {
    ...
    /// When set, [`EvalRunner::run`] clears this collector before each case
    /// so event-aware scorers (`CostScorer`, `LatencyScorer`, `SafetyScorer`)
    /// see only the events from the current case.
...
impl std::fmt::Debug for EvalRunner {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("EvalRunner")
            ...
            .field("event_collector", &self.event_collector.is_some())
            .finish()
    }
}
...
impl EvalRunner {
    ...
    pub fn with_event_collector(mut self, collector: EventCollector) -> Self {
    ...
    pub async fn run<P: crate::llm::LlmProvider>(
        ...
        for case in cases {
            if let Some(collector) = self.event_collector.as_ref() {
...
mod tests {
    ...
    #[tokio::test]
    async fn run_clears_attached_event_collector_between_cases() {
        use crate::llm::LlmProvider;
        use crate::llm::types::{CompletionRequest, CompletionResponse, ContentBlock, StopReason};
        use std::sync::Mutex;
        ...
        // A scorer that records how many events the collector held at score time.
        struct EventCounter {
            collector: EventCollector,
            seen: Arc<Mutex<Vec<usize>>>,
        }
        impl EvalScorer for EventCounter {
            fn name(&self) -> &str {
                "event-counter"
            }
            fn score(&self, _c: &EvalCase, _o: &str, _t: &[String]) -> (f64, Vec<String>) {
                let n = self.collector.lock().expect("counter lock").len();
                self.seen.lock().expect("seen lock").push(n);
                (1.0, vec![])
            }
            fn pass_threshold(&self) -> f64 {
                0.0
        ...
        let collector = EvalRunner::event_collector();
        let agent = crate::agent::AgentRunner::builder(Arc::new(MockProvider))
            .name("eval-isolation")
            .system_prompt("test")
            .max_turns(1)
            .on_event(EvalRunner::event_callback(&collector))
            .build()
            .unwrap();

        let seen = Arc::new(Mutex::new(Vec::<usize>::new()));
        let runner = EvalRunner::new()
            .with_event_collector(collector.clone())
            .scorer(EventCounter {
                collector: collector.clone(),
                seen: seen.clone(),
            });

        let cases = vec![EvalCase::new("c1", "first"), EvalCase::new("c2", "second")];
        let _ = runner.run(&agent, &cases).await;

        // Each case generates the same set of events (RunStarted, LlmRequest,
        // LlmResponse, RunCompleted, ...). Without per-case clearing the
        // second case would see strictly more events than the first.
        let seen = seen.lock().expect("seen lock").clone();
        assert_eq!(seen.len(), 2, "scorer should run once per case");
        assert_eq!(
            seen[0], seen[1],
            "with_event_collector must clear the collector between cases (saw {seen:?})"
        );
        assert!(
            seen[0] > 0,
            "expected at least one captured event per case (saw {seen:?})"
        );
    }
